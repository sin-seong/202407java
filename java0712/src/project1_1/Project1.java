...
//		Member[] list = null;
		...
		for (Member member : list) {
			if(member != null) {// null이 아니면 출력하며 카운팅 하나증가
		...
		while (run) {
			...
			case 1:
				...
				for (int i=0; i < list.length; i++) {
					if (list[i]!= null && list[i].name.equals(name) && list[i].ssn.equals(strPassword)) {
						find = i;
						member = list[i];
						break;
					}
...
//				}
			...
			case 2:
				...
				// 객체 생성
				list[memCnt++] = new Member(name2, ssn, tel);//배열에서 쓰이니 ++을 뒤에 붙임
				break;
			case 3:
				// 예금 출금
				if (member == null) {// 로그인상태가 null이면 출력 *33줄
