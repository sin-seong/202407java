...
String find = request.getParameter("find"); //update하기 전에 학번을클릭하면 넘어오는 매개변수값
String num = request.getParameter("num"); //null 이면 인설트나 업데이트를 하지말아야함
...
//학번이 null이 아니면 이미 있는 학번인지 조회
if(num != null){
	sqlTmp = "select count(*) from score where num = ?";//이미 있는 학번 조회 (있을시 1을 가져옴)
	pstmt = conn.prepareStatement(sqlTmp);
...
while(rs.next()){
	...
	engTmp= rs.getString("eng");
	mathTmp = rs.getString("math");
	int total = Integer.parseInt(korTmp)+Integer.parseInt(engTmp)+Integer.parseInt(mathTmp);
	double avg = total/ 3.0;
%>
	<tr>
	<td><a href="?find=<%=numTmp %>"><%=numTmp %></a></td><td><%=nameTmp %></td><td><%=korTmp %></td>
	<td><%=engTmp %></td><td><%=mathTmp %></td>
