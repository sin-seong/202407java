...
public class BoardDAO {
	...
	private String BOARD_COUNTING = "update board set hits = hits + 1 where num = ?";
	
	
	//게시판 카운팅
	public void countingBoard(int num) {
		conn = JDBCUtil.getConnection();
		try {
			stmt = conn.prepareStatement(BOARD_COUNTING);
			stmt.setInt(1, num);
			stmt.executeUpdate();
...
//public List<BoardDTO> getboarUp(){
//	List<BoardDTO> Up = new ArrayList<>();
//	
//	conn = JDBCUtil.getConnection();
//	try{
//		stmt = conn.prepareStatement(BOARD_UPDATE);
